...
void ControlCompute(void) {
	...
	if (control.status_bits & EMERGENCY_BIT || 
		...
		if (current_goal->start_time < 0){
			current_goal->start_time = now;
		}
...
void goalPwm(goal_t *goal) {
	if ((now - goal->start_time)/1000.0 <= goal->data.pwm_data.time){
		float pwmR, pwmL;
...
void goalSpd(goal_t *goal) {
	if ((now - goal->start_time)/1000.0 <= goal->data.spd_data.time){
		float time_left, v_dec;
		time_left = (goal->data.spd_data.time - ((now - goal->start_time)/1000.0)) / 1000.0;
